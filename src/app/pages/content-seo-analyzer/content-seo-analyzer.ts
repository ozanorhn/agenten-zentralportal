  ...
  private toFriendlyErrorMessage(error: unknown): string {
    if (error instanceof RequestError) {
      switch (error.code) {
        ...
        case 'empty':
          return 'Der Webhook hat keine auswertbare SEO-Antwort zurueckgegeben.';
        default:
          if (error.status === 404) {
            return 'Der Analyse-Webhook wurde nicht gefunden.';
          }

          if (error.status === 500) {
            return 'Der Analyse-Webhook hat mit HTTP 500 geantwortet.';
          }

          return 'Die Analyse konnte nicht gestartet werden. Bitte versuche es erneut.';
      }
    }
